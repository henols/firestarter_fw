...
Defect class this closes: src/eprom_operations.cpp -- the eight eprom_*
...
test_eeprom28c_sdp.cpp; the eight wrapper call sites have no native and no
...
retired eprom_verify (CMD_VERIFY, ordinal 6), taking the census to eight;
it moves again in the same phase, to seven, when eprom_blank_check
(CMD_BLANK_CHECK, ordinal 4) goes.
...
Coverage:
  ...
  3. test_the_eight_forwarding_calls_are_present -- the positive
     counterpart to Coverage 1: op_execute_stateful_operation and
     op_execute_simple_operation, each reached via a bare `return`,
     together occur exactly _EXPECTED_FORWARD_TOTAL (8) times in the
...
def test_the_eight_forwarding_calls_are_present():
    ...
    _EXPECTED_FORWARD_TOTAL (8) times. Phase 204 retired eprom_verify,
    taking the census from Plan 05's original nine to eight."""
    stripped = _strip_comments(_SCAN_WRAPPERS.read_text())
